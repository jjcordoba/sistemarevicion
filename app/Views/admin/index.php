...
class Admin extends BaseController
{
    ...
    public function __construct()
    {
        ...
        if (service('auth')->checkAccess() !== true) {
    ...
    {
        /* if (service('auth')->checkAccess('actualizar configuracion') !== true) {
            return redirect()->to(base_url('sinpermisos'));
        } */

        if (!session()->get('logged_in')) {
            return redirect()->to('/sisesio');
        }
    ...
    {
        if (service('auth')->checkAccess('modificar configuracion') !== true) {
            return redirect()->to(base_url('sinpermisos'));
        }
        ...
        $this->reglas = [
            ...
            'telefono' => [
                'rules' => 'required',
                'errors' => [ 'required' => 'El {field} es requerido' ] ], '
                direccion' => [ 'rules' => 'required',
                 'errors' => [ 'required' => 'El {field} es requerido' ] ], 
                 'cuotas' => [ 'rules' => 'required', 
                 'errors' => [ 'required' => 'El {field} es requerido' ] ], 
                 'mensaje_ticket' => [ 'rules' => 'required', 
                 'errors' => [ 'required' => 'El {field} es requerido' ] ] ];
            ...
            {
                if (service('auth')->checkAccess('ver prestamos vencidos') !== true) {
                    return redirect()->to(base_url('sinpermisos'));
                }
            ...
            {
                if (service('auth')->checkAccess('crear respaldo') !== true) {
                    return redirect()->to(base_url('sinpermisos'));
                }
                ...
                if (file_put_contents($backupPath, $backupData) !== false) {
                    if ($this->crearZip($backupPath)) {
                        ...
                        return redirect()->to(base_url('reditdashboard'))->with('message', 'Error al crear zip');
                    }
                } else {
                    return redirect()->to(base_url('reditdashboard'))->with('message', 'Error al crear el respaldo.');
                }
            }
            ...
            {
                if (service('auth')->checkAccess('ver comparacion') !== true) {
                    return redirect()->to(base_url('sinpermisos'));
                }
